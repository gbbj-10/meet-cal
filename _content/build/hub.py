...
(function(){
  ...
  function showMine(force){
    ...
    if(dg && !dg.dataset.on){
      ...
      /* WebGL 이 아예 없는 기기는 기다리지 않고 바로 접는다(루프20 — 3초 동안 '불러오는 중'만 떠 있었다) */
      var gl=null; try{ var cv=document.createElement('canvas'); gl=cv.getContext('webgl2')||cv.getContext('webgl'); }catch(e){}
      if(!gl){ dg.classList.remove('ld'); dg.style.display='none'; }
      else {
