...
<main class="wrap">
  <section class="hero" id="hero">
    <div class="k">宮合所</div>
    <h1>궁합소</h1>
    <p id="hero-p">상대를 카카오톡으로 불러, 두 사람의 사주 여덟 글자로 궁합을 봐요.
  ...
  <section class="card" id="v-off" hidden>
    ...
    <p class="s">계정 연결이 아직 준비되지 않았어요. 잠시 뒤에 다시 들어와 주세요.</p>
  </section>

  <section class="card" id="v-login" hidden>
    <h2 id="lg-h">카카오톡으로 시작해요</h2>
    <p class="s" id="lg-p">궁합은 두 사람이 각자 로그인해서 봐요. 초대한 사람도, 초대받은 사람도 결과를 같이 볼 수 있어요.</p>
    <button class="btn kakao" id="lg-go">카카오톡 로그인</button>
  </section>

  <section class="card" id="v-birth" hidden>
    <h2 id="bh">내 생년월일</h2>
    <p class="s" id="bp">궁합을 계산할 사주 여덟 글자를 만들어요.</p>
    <div class="frow">
    ...
    <p class="note">생년월일은 저장하지 않고, 계산한 사주 여덟 글자와 오행 비율만 저장해요. 시각을 모르면 '모름'으로 두세요.</p>
  </section>

  <section class="card" id="v-host" hidden>
    <div class="me" id="host-me"></div>
    <button class="btn kakao" id="inv">카카오톡으로 상대 초대하기</button>
    <p class="note">상대가 링크로 들어와 로그인하고 생년월일을 넣으면, 여기에 결과가 생겨요.</p>
...
function birthView(mode){
  ...
  if(mode==='host'){ $('bh').textContent='먼저 내 생년월일을 넣어 주세요';
    $('bp').textContent='궁합을 보려면 내 사주 여덟 글자가 있어야 해요. 한 번만 넣으면 돼요.';
