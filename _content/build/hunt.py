...
function buildFight(){
  var g=CUR, gEl=g.el;
  var team=[{nick:(ME.nick||'나'), el:MYEL, me:true}];
  PARTY.forEach(function(f){ if(f) team.push({nick:f.nick, el:f.el, me:false}); });

  var syn=synergy(team.map(function(t){return t.el}));
  team.forEach(function(t){
    t.hp=t.max=100;
    t.atk=18 + (DAY.els.indexOf(t.el)>=0 ? 3 : 0);   /* 오늘의 기운을 탄 사람은 조금 세다 */
    t.down=false;
  });
  ...
  while(foe.hp>0 && team.some(function(t){return !t.down}) && turn<TC){
    ...
    for(var i=0;i<team.length && foe.hp>0;i++){
      var t=team[i]; if(t.down) continue;
      var r=strike(t.el, foe.el);
      var dmg=Math.max(3, Math.round(t.atk*r.m*bonus*(0.88+Math.random()*0.24)));
      foe.hp=Math.max(0, foe.hp-dmg);
      var line=t.nick+'의 '+J(t.el,'i')+' '+jn(foe.nm,['을','를'])+' 칩니다'+(r.s?' — '+r.s:'');
      push(line+' (−'+dmg+')', r.k);
      L[L.length-1].foe=foe.hp/foe.max;
      act({t:'hit', by:i, dmg:dmg, kind:(r.m>=1.5?'big':''), foeHp:foe.hp/foe.max, text:line});
    }
    if(foe.hp<=0) break;
    var alive=team.filter(function(t){return !t.down});
    var v=alive[Math.floor(Math.random()*alive.length)];
    var r2=strike(foe.el, v.el);
    var d2=Math.max(2, Math.round(foe.atk*r2.m*(0.85+Math.random()*0.3)));
    v.hp=Math.max(0, v.hp-d2);
    var line2=jn(foe.nm,['이','가'])+' '+jn(v.nick,['을','를'])+' 덮칩니다'+
              (r2.m>1.3?' — 눌립니다':'');
    push(line2+' (−'+d2+')', r2.m>1.3?'bad':'');
    L[L.length-1].team=team.map(function(t){return {hp:t.hp/t.max, down:t.down}});
    act({t:'foehit', to:team.indexOf(v), dmg:d2, kind:(r2.m>1.3?'big':''),
         hp:team.map(function(t){return t.hp/t.max}), text:line2});
    if(v.hp<=0){ v.down=true;
      var line3=jn(v.nick,['이','가'])+' 쓰러졌습니다.';
      push(line3,'bad');
      L[L.length-1].team=team.map(function(t){return {hp:t.hp/t.max, down:t.down}});
      act({t:'down', i:team.indexOf(v), text:line3}); }
  }

  var win=foe.hp<=0;
  var standing=team.filter(function(t){return !t.down}).length;
  var endLine = win ? jn(foe.nm,['이','가'])+' 물러갑니다.' : '더는 버틸 수 없습니다. 물러납니다.';
  ...
  else if(win && n>=3)            why='머릿수가 갈랐습니다. '+n+'명이 번갈아 치니 버티지 못했습니다.';
  else if(win)                    why='아슬아슬했습니다. 한 명만 더 있었으면 편했을 겁니다.';
  else if(rel && rel.k==='risk')  why=J(gEl,'i')+' 내 '+J(MYEL,'eul')+' 누르는 곳입니다. 나를 살려 줄 기운을 데려오세요.';
  else if(n===1)                  why='혼자서는 무리였습니다. 친구를 부르면 달라집니다.';
