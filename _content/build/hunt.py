...
function buildFight(){
  ...
  var L=[], turn=0, bonus=1+syn.bonus/200;
  function push(t,k){ L.push({t:t,k:k||''}) }
  ...
  while(foe.hp>0 && team.some(function(t){return !t.down}) && turn<24){
    ...
    for(var i=0;i<team.length && foe.hp>0;i++){
      ...
      push(t.nick+'의 '+J(t.el,'i')+' '+jn(foe.nm,['을','를'])+' 칩니다'+(r.s?' — '+r.s:'')+
           ' (−'+dmg+')', r.k);
    ...
    push(jn(foe.nm,['이','가'])+' '+jn(v.nick,['을','를'])+' 덮칩니다'+
         (r2.m>1.3?' — 눌립니다':'')+' (−'+d2+')', r2.m>1.3?'bad':'');
    L[L.length-1].team=team.map(function(t){return {hp:t.hp/t.max, down:t.down}});
    if(v.hp<=0){ v.down=true; push(jn(v.nick,['이','가'])+' 쓰러졌습니다.','bad');
      L[L.length-1].team=team.map(function(t){return {hp:t.hp/t.max, down:t.down}}); }
  }

  var win=foe.hp<=0;
  var standing=team.filter(function(t){return !t.down}).length;
  push(win ? jn(foe.nm,['이','가'])+' 물러갑니다.' : '더는 버틸 수 없습니다. 물러납니다.',
       win?'big':'bad');
  ...
  return {log:L, win:win, turns:turn, standing:standing, total:n, why:why,
...
function endBattle(F){
  ...
  $('b-party').onclick=function(){ if(BT){clearTimeout(BT);BT=null;} show('v-party'); };
  $('b-ground').onclick=function(){ if(BT){clearTimeout(BT);BT=null;} renderPick(); };
}
