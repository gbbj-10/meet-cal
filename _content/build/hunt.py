...
.spd{position:absolute;top:8px;right:8px;display:flex;gap:6px;z-index:5}
.spd button{min-height:36px;min-width:44px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(10,14,22,.72);backdrop-filter:blur(3px);
  color:var(--mut);font:inherit;font-size:13px;font-weight:700;cursor:pointer;padding:0 10px}
.spd button.on{background:#ffd93d;color:#191600;border-color:#ffd93d}
#b-skip{margin-left:4px}
...
<div class="wrap">
  ...
  <section class="view" id="v-battle">
    ...
    <div class="stagew"><div class="stage" id="stage"><div class="load" id="load">사냥터를 그리는 중…</div></div>
      <div class="spd" id="spd"><button data-s="1">×1</button><button data-s="2">×2</button><button data-s="3">×3</button><button id="b-skip">건너뛰기 ▶▶</button></div></div>
...
function buildFight(){
  ...
  var margin=Math.max(1, P.pow/need);           /* 문턱을 얼마나 넘었나 */
  ...
  push('전투력 '+P.pow+' · 권장 전투력 '+need+(P.pow>=need*1.3?' — 여유 있습니다.':''),'hit');
  ...
  /* 우리 쪽 공격 몫 — 각자의 수치와 상성으로 나누고, T 번에 걸쳐 정확히 다 깎는다 */
  var share=team.map(function(t,i){ return P.vals[i]; });
  var shareSum=share.reduce(function(a,b){return a+b},0);
  var plan=[], tot=0;
  for(var r=0;r<T;r++) for(var i=0;i<n;i++){ var d=share[i]/shareSum*foe.max/T*jit(); plan.push(d); tot+=d; }
  plan=plan.map(function(d){ return Math.max(1,Math.round(d*foe.max/tot)); });
  var fix=foe.max-plan.reduce(function(a,b){return a+b},0); plan[plan.length-1]+=fix;
  ...
  for(var turn=1; turn<=T; turn++){
    ...
    for(var i=0;i<n;i++){
      ...
      act({t:'hit', by:i, dmg:dmg, kind:(sk.m>=1.5?'big':''), foeHp:foe.hp/foe.max, text:line});
      if(foe.hp<=0) break;
    }
    if(foe.hp<=0) break;
  ...
  var why = !nxt ? '갑단 정복 — 이 사냥터의 끝까지 올랐습니다.'
          : gapN<=0 ? '여유 있게 이겼습니다. '+tOf(K+1).nm+'이 이미 열려 있습니다.'
          : tOf(K+1).nm+'까지 전투력 '+gapN+' 남았습니다 — '+FP.edge(gEl)+' 용신석이 가장 빨리 채워 줍니다.';

  return {log:L, acts:A, win:true, turns:T, standing:n, total:n, why:why,
          foe:foe, team:team, syn:{pairs:P.pairs}, tier:K, loot:loot, lootEl:gEl, P:P, need:need};
}
...
var SPEED=LS.get('hunt.speed',1)||1;
function setSpeed(v){ SPEED=v; LS.set('hunt.speed',v);
  [].forEach.call(document.querySelectorAll('#spd [data-s]'),function(b){ b.classList.toggle('on', +b.dataset.s===v); }); }      /* 글로 흐를 때의 타이머 */
...
function playBattle(){
  ...
  show('v-battle'); setSpeed(SPEED);
  [].forEach.call(document.querySelectorAll('#spd [data-s]'),function(b){ b.onclick=function(){ setSpeed(+b.dataset.s); }; });
  $('b-skip').hidden=false;
  $('b-skip').onclick=function(){ if(!CURF) return; var f=CURF; CURF=null; stopBattle();
    $('stage').hidden=true; ev('hunt_skip',{}); endBattle(f); };
  renderBattleHead(F);
...
function playLog(F){
  ...
  (function step(){
    ...
    if(l.foe!=null) $('foe-hp').style.width=Math.round(l.foe*100)+'%';
...
function endBattle(F){
  ...
  $('res').innerHTML=
    ...
    '</div>';
  LS.set('hunt.last.'+CUR.id, {win:true, turns:F.turns, total:F.total, tier:F.tier, at:Date.now()});
  ev('hunt_result',{ground:CUR.nm, tier:F.tier, win:1, turns:F.turns, size:F.total, loot:F.loot, pow:F.P.pow});
...
function renderTier(){
  ...
  TIERS.forEach(function(t){
    var need=TH[t.k-1], isOpen=P.pow>=need, isDone=t.k<=done;
  ...
  $('tnote').innerHTML = tip +
    '<b>권장 전투력을 넘은 단은 반드시 깹니다.</b> '+TIERS.map(function(t){return t.nm+' '+TH[t.k-1]}).join(' · ')+'.<br>'+
...
function renderParty(){
  ...
  var cap2=tierCap(g.el), tooHigh=TIER>cap2;
