...
var sb = null;
if (MODE==='supabase' && window.supabase) sb = window.supabase.createClient(CFG.sbUrl, CFG.sbKey);
...
var Store = {
  ...
  friends:function(){
    ...
    return sb.auth.getUser().then(function(r){
      var u=r.data&&r.data.user; if(!u) return [];
      return sb.from('hunt_invites').select('joined_id,joined_nick,joined_element')
               .eq('host_id',u.id).then(function(q){
        return (q.data||[]).map(function(x){
          return {id:x.joined_id, nick:x.joined_nick, el:x.joined_element};
        });
      });
    });
  },
  ...
  join:function(hostCode, me){
    if(MODE==='demo'){
      ...
      var hit=false;
      f=f.map(function(x){ if(x.id===me.id){ hit=true; return {id:me.id,nick:me.nick,el:me.el}; } return x; });
      if(!hit) f.push({id:me.id,nick:me.nick,el:me.el});
      LS.set('hunt.friends.'+hostCode,f);
      return Promise.resolve();
    }
    return sb.from('hunt_invites').upsert({host_code:hostCode, joined_id:me.id,
      joined_nick:me.nick, joined_element:me.el});
  },
  /* 파티 구성 저장/불러오기 */
  party:function(gid){
...
var TH = (window.FP && FP.TH) ? FP.TH : [50,55,65,85,120,175,255,365,520,730];
...
function keyOf(gEl){ for(var a in DGEUK) if(DGEUK[a]===gEl) return a; return gEl; }

/* 한 사람의 속성 수치. 나는 me.js 에서 정확히, 친구는 받은 값이 있으면 그것,
   없으면 주 속성만 보고 어림한다(주 속성 70, 나머지 55).
   친구 수치를 서버에 올리기 전까지의 임시 규칙이다. */
function statOf(who, x){
  if(who.me){
    var st = window.FP && FP.stats ? FP.stats() : null;
    return st ? st[x].val : 50;
  }
  if(who.stats && who.stats[x]!=null) return who.stats[x];
  return who.el===x ? 70 : 55;
}

/* 파티 수치 — 전부 같은 규칙으로 계산한 한 숫자 */
function powerAt(gEl){
  ...
  var vals=who.map(function(w){ return statOf(w,x); });
  var top=Math.max.apply(null,vals), sum=vals.reduce(function(a,b){return a+b},0);
  var syn=synergy(who.map(function(w){return w.el}));
  var today=(DAY.els.indexOf(x)>=0);
  var pow=(top + (sum-top)*0.2) * (1+0.05*syn.pairs) * (today?1.05:1);
  return {key:x, pow:Math.round(pow), mine:vals[0], top:top, n:who.length,
          pairs:syn.pairs, today:today, who:who, vals:vals};
}
...
function buildFight(){
  ...
  push(P.key+' 수치 '+P.pow+' — 문턱 '+need+(P.pow>=need*1.3?' 을 넉넉히 넘습니다.':' 을 넘습니다.'),'hit');
  if(P.pairs) push('파티의 기운이 서로를 살립니다. 상생 '+P.pairs+'쌍 +'+(P.pairs*5)+'%.','hit');
  if(P.today) push('오늘의 기운이 '+P.key+'에 실려 있습니다. +5%.','hit');

  /* 우리 쪽 공격 몫 — 각자의 수치와 상성으로 나누고, T 번에 걸쳐 정확히 다 깎는다 */
  var share=team.map(function(t,i){ return P.vals[i]*strike(t.el,gEl).m; });
  ...
  var why = margin>=1.3 ? P.key+' 수치 '+P.pow+'이 문턱 '+need+'을 넉넉히 넘었습니다.'
          : margin>=1.1 ? P.key+' 수치 '+P.pow+'로 문턱 '+need+'을 넘었습니다.'
...
function endBattle(F){
  ...
  var st = window.FP && FP.stats ? FP.stats() : null;
  var mineNow = st ? st[F.lootEl].val : null;
  ...
  $('res').innerHTML=
    '<div class="res win">'+
      ...
      '<div class="rn"><span><b>'+F.turns+'</b>번 겨룸</span>'+
        '<span>'+F.P.key+' 수치 <b>'+F.P.pow+'</b> / 문턱 '+F.need+'</span>'+
...
function renderTier(){
  ...
  $('tsub').innerHTML='이 땅은 <b>'+P.key+'</b>이 누릅니다 — <b>'+P.key+' 수치</b>로 싸웁니다. '+
    ...
    (P.n>1&&P.pairs?' (상생 '+P.pairs+'쌍 포함)':'')+(P.today?' · 오늘의 기운 +5%':'')+'.';

  /* 주머니 — 속성 막대 다섯 줄을 그대로 보여 준다 */
  $('pouch').innerHTML = (window.FP && FP.barsHTML) ? FP.barsHTML(P.key) : '';
  ...
  $('tnote').innerHTML =
    '<b>수치가 문턱을 넘은 단은 반드시 깹니다.</b> 계단 50 · 임단 55 · 신단 65 · 경단 85 · 기단 120 · '+
    '무단 175 · 정단 255 · 병단 365 · 을단 520 · 갑단 730.<br>'+
    '<b>'+g.el+' 용신석</b>은 이곳에서 나고, '+g.el+' 수치는 <b>'+((window.FP&&FP.USE)?FP.USE[g.el]:'')+'</b>에서 쓰입니다. '+
...
function afterLogin(me){
  ...
  $('me').innerHTML=(ME.pic?'<img src="'+esc(ME.pic)+'" alt="">':'')+esc(ME.nick)+
                    ' <a id="lo" style="cursor:pointer">나가기</a>';
  var lo=$('lo'); if(lo) lo.onclick=function(){ Store.logout().then(function(){ location.href='/hunt/'; }); };
