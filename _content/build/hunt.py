...
function buildFight(){
  ...
  var why = !win ? '권장 전투력 '+need+'에 '+(need-P.pow)+' 모자랍니다 — '+FP.edge(gEl)+' 용신석 '+Math.ceil((need-P.pow)/1.5)+'개쯤이면 넘습니다.'
          ...
          : gapN<=0 ? '여유 있게 이겼습니다. '+tOf(K+1).nm+'이 이미 열려 있습니다.'
...
function renderPick(){
  ...
  GROUNDS.forEach(function(g){
    ...
    var why='내 전투력 <b>'+P0.pow+'</b> · 다음 '+tOf(nx).nm+' 권장 '+nd+
            (P0.pow<nd ? ' <b style="color:#e8483c">('+g.el+' 용신석 '+Math.ceil((nd-P0.pow)/1.5)+'개 더)</b>' : '')+
            (dn ? ' · '+tOf(dn).nm+' 정복' : '');
...
function renderTier(){
  ...
  TIERS.forEach(function(t){
    ...
    var why = beastOf(g,t.k)+(cnt>1?' '+cnt+'마리':'')+' · 권장 '+need+
      (weak ? ' · <b style="color:#e8483c">'+g.el+' 용신석 '+Math.ceil(gap/1.5)+'개 더</b>'
